...
>
    {openFiles.map((file) => (
        ...
        key={file.id}
        className={cn(
            "flex w-fit cursor-pointer items-center rounded-t-md px-3 py-1 transition-all",
            file.id === activeFile?.id
            ? "bg-gray-700 text-white font-bold"  // Active file (cyan background, black text)
            : "bg-gray-700 text-black hover:bg-gray-600"  // Inactive file (gray background, white text)
    ...
    >
        ...
        <Icon
            ...
            fontSize={22}
            className="mr-2 min-w-fit text-gray-300"
        />
        ...
        {/* Close Button */}
        <IoClose
            className="ml-3 inline rounded-md text-gray-400 hover:text-white"
            size={20}
